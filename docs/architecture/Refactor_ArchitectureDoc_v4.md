...
>
> **v4.1 catalog blueprints (dev/FSE boundary).** The developer **catalog** exposes two named layers beyond `Pipelines` + `Codecs`: **`Formats`** (a per‑leg encoding bundle — a message codec + an optional batch codec) and **`Workflows`** (a contract blueprint — a shared pipeline + a default format). An FSE contract picks a **`Workflow`** by name and owns only **message‑level/operational** settings (`Acknowledgement`/`Response`, `Retry`, `DeliveryGuarantee`, `Channel`, and batch *triggers*); the developer owns the **plug‑and‑play code** concerns (stages, encoding, batch codec). A **`ContractWorkflowResolver`** flattens the workflow/format references into concrete per‑leg values before compilation, and every catalog‑supplied value stays **optionally overridable** on the contract (§8).
>
> **v4.2 Workflow + Settings model (ADR 0001, implemented 2026‑08).** The catalog blueprint is renamed **`Template` → `Workflow`** and gains three capabilities (see `docs/adr/0001-workflow-settings-and-config-templates.md`): **(1)** a Workflow may declare an **ordered `Formats` set** (element 0 is the default; when there are several, each output picks one by name and an output that omits it falls back to the first, logged); **(2)** a **`Settings`** block — a flat, friendly, guard‑railed FSE‑facing form the developer designs, whose values **bind** onto contract fields (`Acknowledgement.TimeoutMs`, `Outputs[].Retry.MaxAttempts`), stage parameters (`stage:<name>.<key>`), or resources (`Kind: file`/`secret`), with `Scale`/`Min`/`Max`/`Allowed`; `ReplyOnFilter` is now one such Setting; **(3)** **`Resources`** (developer‑shipped files a `Kind: file` Setting points at, confined to an allowed root, recorded in a resolved manifest). The contract's `Workflow` reference is **always an object** — `"Workflow": { "Use": "adt", "Settings": { … } }` — and stages receive their bound parameters via `ComponentRegistry.CreateStage(name, StageParameters)`. A generated **`config/templates/`** tree of copy‑paste starters replaces the old `template.json` (§8).
>
> **v4.3 Inline retry + store‑and‑forward corrections (implemented 2026‑08).** The per‑leg **inline retry decorator (`RetryingOutboundEndpoint`)** promised in §3.7 is now implemented — hand‑rolled `Fixed`/`Exponential` backoff (no Polly), **off by default** (`Retry.MaxAttempts` defaults to `1`), and **suppressed on replay** so the durable `ForwardWorker` stays the sole retry authority. The store‑and‑forward replay path was **corrected** (§3.9): the **`ForwardWorker` is the single authority** for `ResolveAsync`/`RescheduleAsync`/`ParkAsync`, all keyed by the store **`Id`** (the previous message‑identity match never matched a rebuilt replay, so a delivered entry was never cleared); replay **targets only deliver‑or‑throw**, and in‑process `DeliveryLeg.ReplayAsync` now **delivers directly, bypassing the leg queue** (it was a fire‑and‑forget re‑enqueue that hid the outcome, so the cap/backoff/park never applied in‑process). Consequence: replays are worker‑serialized and **do not preserve per‑key ordering** for `Ordered` legs (deferred). Reply policy is unchanged — `AtMostOnce` drops + NACKs on failure; `AtLeastOnce` stores + NACKs (the same generated NACK) while the engine keeps retrying.
...
- **Note (naming):** distinct from the config **Catalog** (§8, named pipelines, codecs, formats, and workflows) and the **Component Registry** (§3.10, type→impl factories). Three different lookups, three different names — see §3.10.
...
- **Decorators (compose around the transport endpoint, OCP):** **`RetryingOutboundEndpoint`** — *implemented* — is the inline per‑leg retry layer: it retries a `Failed` outcome or a thrown transport exception up to `Retry.MaxAttempts` with `Fixed`/`Exponential` backoff, then yields the last result (an exhausted `AtLeastOnce` leg then falls through to store‑and‑forward, §3.9). It is **hand‑rolled** (mirrors the `ForwardWorker` backoff — no Polly dependency), **off by default** (`Retry.MaxAttempts` defaults to `1`; a contract opts in per output leg), and **suppressed on replay** (`ctx.IsReplay` ⇒ a single attempt, so the durable `ForwardWorker` stays the sole retry authority for stored messages and inline attempts never compound with durable ones). It is composed in `ComponentRegistry.CreateOutboundEndpoint`, wrapping the bare endpoint — while lifecycle/duplex wiring keep tracking the bare instance via `OutboundEndpointLifecycles`, so wrapping hides nothing. `BatchingOutboundEndpoint` (size/time flush) and `TelemetryOutboundEndpoint` are the other planned decorators.
...
- **Reuse the engine's delivery path — no duplicate senders.** Replays go through the **same** `IOutboundEndpoint` + codec the engine uses; the legacy duplicated `TcpRetrySender`/`HttpRetrySender` are deleted. In‑process the worker calls `DeliveryLeg.ReplayAsync`, which **delivers straight through the leg's endpoint — bypassing the leg's queue** — and reports success (returns) or failure (throws) back to the worker; out‑of‑process an `EndpointReplayTarget` wraps an identically‑composed endpoint and does the same. In both modes the target **only delivers**; resolve/reschedule/park is the worker's job. Both honor the replay invariants.
- **One store, one worker, one active owner.** The store lives in the shared `Philips.IBE.IBEAgent.Persistence` library; the `ForwardWorker` is **one implementation** with **two hosting modes**, of which **exactly one is the active owner** (config `Forward:Owner`): `ForwardService` (default — out‑of‑process in `Philips.IBE.IBEAgent.ForwardService`, the modern replacement for the legacy `CimS3ForwardService`; survives agent downtime, isolates heavy S3/CIM retry) **or** `InProcess` (co‑located in the agent; lowest latency, no extra process). The **store is the seam**; a row lease (`SELECT … FOR UPDATE SKIP LOCKED`) is a **safety net** (not the primary mechanism) so an accidental double‑enable can never double‑send. Inline transient retry (seconds) always lives in the leg's decorator regardless of owner; the store‑driven `ForwardWorker` is only the *durable, exhausted‑retry* half.
...
3. **Reuse the engine's delivery stack.** Replay delivers through the **same composed `IOutboundEndpoint` + codec (+ the Retry decorator)**, built by the **same factory from the same config** — in‑process via `DeliveryLeg.ReplayAsync` (which **bypasses the leg queue** and delivers directly; the inline Retry decorator is suppressed on replay so the durable worker is the sole retry authority), out‑of‑process via an identically‑composed endpoint. The legacy `TcpRetrySender`/`HttpRetrySender` stay deleted.
...
- **Replays bypass the leg queue → an ordering caveat.** A replay now delivers **directly through the endpoint** (the worker serializes replays one at a time) rather than re‑entering the leg's queue, so strict **per‑key ordering is not preserved on replay** for `Ordered` legs — a re‑sent message can land after a later fresh one. This is an accepted limitation while `Ordered` (partition‑by‑key, park‑and‑halt, §5) is deferred; when it lands, ordered legs will need a queue‑based (carry‑the‑entry) replay path. Out‑of‑process forwarding likewise stays **unordered‑only** (the compiler rejects `Ordered` + `Forward:Owner=ForwardService`).
- **Crash‑safe outbox ordering:** the worker delivers → confirms → *then* `ResolveAsync(id)` (delete by store id). A crash between send and delete re‑delivers on restart (at‑least‑once) — it **never loses** — and the downstream `DeduplicateStage` / `IdempotencyKey` absorbs the rare duplicate; `ResolveAsync` is idempotent (removing an unknown/already‑removed id is a no‑op).
...
- **Configuration subsystem** (§8), split into two layers so the Web service can share it without pulling in the engine:
  - **`Philips.IBE.IBEAgent.Configuration` (pure, shared):** typed option DTOs, the **Catalog** DTOs (named pipelines, codecs, **formats**, **workflows**, and **resources**), the **`ContractWorkflowResolver`** (flattens a contract's `Workflow`/`Format` references into concrete per‑leg encodings, binds the FSE `Settings` bag onto contract/stage targets, and resolves `Kind:file`/`secret` resources before compilation), `IValidateOptions` **structural** validators (unique ids, single‑format INV‑2, ack XOR response, capacity/DOP > 0, referential integrity, workflow/format resolution), and the generated **JSON schema/manifest**. Depends only on `Abstractions`. Referenced by **both** the agent Host **and** the Web service → one source of config truth.
...
stateDiagram-v2
    ...
    StoredForForward --> Sending: ForwardWorker replay (direct, same leg)
...
**Filtered messages (shared‑pipeline short‑circuit).** A message dropped by the pipeline (filter/dedup) reports `Filtered` to the `ReplyContext`. Whether the source is told is decided by **`ReplyOnFilter`** — a **developer default expressed as a Workflow `Setting`** (ADR 0001; the developer who defines the filter pipeline best knows the intent), which an **FSE may override** via the contract's `Settings`. **`true`** sends an intentional‑**reject** ack carrying the filter reason (HL7 **`AR`** + reason — distinct from a delivery‑failure `AE`); **`false`** (the default when unset) is a **silent drop** (no reply, and the pending reply timer is cancelled), reproducing the legacy behavior. The reject *code* is the formatter's job (keyed on the `Filtered` outcome), so no HL7 specifics leak into the engine.
...
**Message format & ack shape (single‑format contracts).** Each **input comm point** declares one `Format` (INV‑1: one input → one format; `hl7v2` today, `fhir`/`xml` later) that selects the parser, stages, and source `IAckFormatter`. **All inputs of a contract must share the same `Format`** (INV‑2) — validated at load; two formats ⇒ two contracts. A contract's `Acknowledgement` sets `AckShape` (`Single` default | `Batch`) — a **config choice**, not auto‑detected. Symmetrically each **output leg's wire encoding** comes from the contract's **`Workflow`** (its default `Format`, or a member of its `Formats` set) — a **developer** decision, not an FSE‑authored field; a leg may optionally override it with a different catalog `Format`, or (legacy escape hatch) an inline `Encoding` codec name. Per **INV‑4** the reply is never converted; per **INV‑5** the pipeline parses once and codecs serialize per leg.
...
**Contract** = a **`Workflow`** reference (always an object `{ Use, Settings }`; the developer blueprint supplying the shared **pipeline** and the default per‑leg **format**/encoding), `Inputs` (one entry per input comm point, each with its own `Channel` — symmetric with outputs), `Acknowledgement { IsEnabled, IsEnhanced }` **or** a `Response` block (the FSE‑owned reply mode, mutually exclusive), and an **`Outputs`** list. (Filter‑reply behavior — `true` = a pipeline‑filtered message gets an intentional‑reject ack, `false` = silent drop — is a Workflow `Setting` binding `ReplyOnFilter`, not a hand‑authored contract field.) Each **input** carries its own `InputId` and `Channel { Capacity, DegreeOfParallelism, Ordered, OverflowPolicy }` (per‑input isolation). Each **output** carries its **FSE‑owned** `Required`, `DeliveryGuarantee`, `Channel { Capacity, DegreeOfParallelism, Ordered, OverflowPolicy }`, `Retry`, batch **triggers** (`Batching { Enabled, MaxCount, MaxLatencyMs }`), and an optional **`FromInputIds`** (the subset of this contract's inputs that route to this output; null/empty = all inputs — default); its **wire encoding and batch codec are inherited** from the Workflow's `Format`, with an optional per‑leg `Format` override (developer‑named) or an inline `Encoding` (legacy escape hatch). A **workflow‑less** contract may instead name a `Pipeline` and inline `Encoding` per output directly (manual mode). **There is no per‑output pipeline** (YAGNI) — output formatting is the endpoint's codec, chosen by the resolved `Encoding` (per message) or the resolved batch codec (per batch); all message processing already happened once in the shared pipeline.

**Catalog (developer‑owned, e.g. `catalogData.json`):** the single place developers define reusable, named building blocks that FSEs reference by name — **`Pipelines`**, **`Codecs`**, **`Formats`**, **`Workflows`**, and **`Resources`**. Each layer references the previous by name (`Workflows`→`Pipelines`+`Formats`; `Formats`→`Codecs`).
...
- **`Formats`** — named **per‑output‑leg encoding bundles**: a message `Codec` plus an optional `BatchCodec` (both names of `Codecs` entries). The developer's "how a leg renders bytes" unit — an FSE output inherits it from the workflow or names one to override a single leg. (The name is distinct from an *input comm point's* `Format` tag, which selects the parser/ack; this `Formats` catalog governs *output* rendering.)
- **`Workflows`** — named **contract blueprints** an FSE picks by name: a shared **`Pipeline`** (optional; omit = no stages), a default per‑leg **`Format`** *or* an ordered **`Formats`** set (element 0 = default; with several, each output picks a member and an omitted one falls back to the first, logged), an optional **`Version`**, and a **`Settings`** block. Message‑level QoS (ack mode, retry, delivery guarantee, channel, batch triggers) stays FSE‑owned on the contract, but the Workflow may **delegate** a bounded slice of it through Settings (below).
- **`Settings`** (on a Workflow) — the flat, friendly FSE‑facing form the developer designs. Each entry has a friendly name + optional `Default`, guardrails (`Min`/`Max`/`Allowed`/`Regex`), a hidden **`Bind`** target (a contract field like `Acknowledgement.TimeoutMs`, a per‑output wildcard `Outputs[].Retry.MaxAttempts`, or a stage parameter `stage:<name>.<key>`), an optional **`Kind`** (`file`/`secret`) + `ContentType`, and a numeric **`Scale`**. A Setting with no `Default` is **required**; anything not declared as a Setting is constant and invisible to the FSE. `ReplyOnFilter` is expressed here. (Full model: ADR 0001.)
- **`Resources`** — named developer‑shipped files (`{ ContentType, Ref }`) a `Kind: file` Setting points at. At resolution the value (a resource name or a path) is confined to an allowed root (traversal / absolute escape rejected) and recorded in a resolved manifest; `Kind: secret` values resolve from the secret store and are never listed.
```jsonc
// catalogData.json  (developers own this; FSEs only reference Workflow names)
{
...
**Backward‑compatible shorthand:** a flat `InputIds: [1, 2]` array (without per‑input `Channel`) compiles to `Inputs` entries with default `Channel` settings; a single `OutputId` compiles to a one‑element `Outputs` list (one required leg); an omitted/null shared `Pipeline` means **no processing stages** — the message is delivered as received (the leg's endpoint still encodes + sends). A contract may omit `Workflow` entirely and wire developer concerns inline (a `Pipeline` name + per‑output `Encoding`) — **manual mode**. Before compilation the **`ContractWorkflowResolver`** flattens any `Workflow`/`Format` references, binds the FSE `Settings`, resolves `Kind:file`/`secret` resources, into concrete per‑leg `Pipeline`/`Encoding`/batch‑codec values, so the compiler and validators always see fully‑resolved contracts.

**Example `contractData.json` (FSE‑owned) — pick a `Workflow` by name; the FSE owns only message‑level/operational settings:**
```jsonc
{
  "Contracts": [
    {
      "Name": "adt-fanout",
      "Workflow": { "Use": "adt" },                              // developer blueprint: pipeline "adt-standard" + default format "hl7-standard"
      ...
      "Outputs": [
        {
          ...
          "DeliveryGuarantee": "AtMostOnce",                       // encoding inherited from Workflow -> Format "hl7-standard" (hl7v2)
    ...
    },
    {
      ...
      "Workflow": { "Use": "lab-query" },                        // pipeline "query-validate" + default format "hl7-standard"
      "Inputs": [
      ...
      "Outputs": [
        {
          ...
          "DeliveryGuarantee": "AtLeastOnce",                     // encoding inherited from Workflow; FORWARD retried/guaranteed, the RETURN reply is ephemeral
...
- **New output wire format / encoding** → implement `IMessageCodec` (per message) or `IBatchCodec` (per batch), register the type, add a named entry to the catalog's `Codecs` (type + params), bundle it into a `Formats` entry, and expose it through a `Workflows` entry (or reference the `Format` directly on an output). FSE contracts pick it by **workflow name**. No engine change.
- **New reusable contract blueprint** → add a `Formats` and/or `Workflows` entry to the catalog (config only) that composes existing pipelines + codecs; FSEs reference it by name. No engine change.
...
class IForwardStore { <<one store: Pending|Parked, tagged by OutputId>> +StoreAsync(ctx,outputId,error) +ResolveAsync(id) +ParkAsync(id,reason) }
...
public sealed class Output
{
    ...
    // Leg-targeted replay: delivers straight through the endpoint (BYPASSES the queue) and reports the
    // outcome to the ForwardWorker by throwing on failure. Resolve/reschedule/park is the worker's job.
    public async ValueTask ReplayAsync(MessageContext ctx, CancellationToken ct)
    {
        ctx.MarkReplay();                                       // Retry decorator -> single attempt; no reply reporting
        var result = await _endpoint.SendAsync(ctx, ct);
        if (result.Outcome != DeliveryOutcome.Delivered)
            throw new InvalidOperationException(result.Error);  // worker reschedules/parks by store id
    }

    public async Task RunAsync(CancellationToken ct)            // FRESH messages only; replays never enter this queue
    {
        await foreach (var ctx in _queue.ReadAllAsync(ct))
        {
            ...
            if (result.Outcome != DeliveryOutcome.Delivered && _forward is not null)
                await _forward.StoreAsync(ctx, OutputId, result.Error); // AtLeastOnce -> Pending; ForwardWorker replays into THIS leg
            ctx.Reply.ReportLeg(Required, result);                // one reply per fresh message
...
| **2. Configuration** | Pure option + Catalog DTOs (`Pipelines`/`Codecs`/`Formats`/`Workflows`/`Resources`), the **`ContractWorkflowResolver`** (flatten `Workflow`/`Format` → concrete per‑leg encoding, bind `Settings`, resolve resources), and `IValidateOptions` structural validators (INV‑2, ack XOR response, capacity/DOP > 0, referential integrity, `FromInputIds`, workflow/format resolution, encoding⇄format) + JSON schema/manifest. | `Configuration` | 1 | validators reject each INV violation; resolver flattens workflows + binds settings; round‑trips the example `contractData.json`/`catalogData.json` | one config truth shared by Agent + Web |
