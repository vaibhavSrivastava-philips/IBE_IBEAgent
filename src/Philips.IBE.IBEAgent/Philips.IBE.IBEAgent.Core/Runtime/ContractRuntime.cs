    ...
    private async Task ConsumeAsync(IMessageChannel ingress, CancellationToken cancellationToken)
    {
        await foreach (var ctx in ingress.ReadAllAsync(cancellationToken))
        {
            ...
            if (pipeline.ShortCircuited)
            {
                ...
                // Routine, by-design drop (a filter's job is to drop). Logged at Info as a TERMINAL
                // disposition so the per-message flow is complete for monitoring: every received message
                // ends as delivered | filtered | failed. A very high-volume filter can raise this logger to Warn.
                _logger.LogInformation(
                    "Message {CorrelationId} for contract {ContractName} from source {SourceEndpointId} filtered: {Reason}",
                    ctx.CorrelationId, _contractName, ctx.SourceEndpointId, pipeline.Reason ?? "unspecified");
