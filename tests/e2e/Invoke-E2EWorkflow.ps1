...
.DESCRIPTION
    ...
    output transport (TCP, HTTP), and acknowledgement mode (none, normal,
    enhanced, response), plus fan-out topologies. For each scenario the workflow:
...
function Invoke-Scenario {
    ...
    try {
        ...
        $inboundPort = if ($Scenario.Input -eq 'tcp') { 16000 + $Index } else { 18000 + $Index }
        $ready = $false
        ...
        foreach ($out in $Scenario.Outputs) {
            $captureFile = if ($out -eq 'tcp') { $TcpCaptureFile } else { $HttpCaptureFile }
        ...
        switch ($Scenario.Ack) {
            ...
            'response' {
                $marker = if ($Scenario.Outputs -contains 'tcp') { $DownstreamTcpMarker } else { $DownstreamHttpMarker }
